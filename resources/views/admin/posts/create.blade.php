...
<div class="max-w-2xl mx-auto">
    <h1 class="text-2xl font-bold text-slate-900 mb-6">Nouvel article</h1>

    <form method="POST" action="{{ route('admin.posts.store') }}" class="bg-white rounded-lg border border-slate-200 p-6">
        @csrf
        <div class="mb-4">
        ...
        <div class="mb-4">
            ...
            <textarea name="excerpt" rows="2" class="w-full border border-slate-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900"></textarea>
        </div>

        <div class="mb-4">
            <label class="block text-sm font-medium text-slate-700 mb-1">Contenu</label>
            <textarea name="content" rows="8" required class="w-full border border-slate-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900"></textarea>
        </div>
        ...
        <div class="flex items-center gap-3">
            <button type="submit" class="bg-slate-900 text-white px-4 py-2 rounded-md text-sm hover:bg-slate-700">Enregistrer</button>
