...
@section('title', 'Arborescence documentaire — La Main à la Pâte')

@section('content')
<div class="max-w-6xl mx-auto px-4 py-8" x-data="documentTree('{{ route('documents.tree.data') }}')">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-slate-900">Arborescence documentaire</h1>
        <p class="text-slate-500 text-sm mt-1">Sources, annexes et OCR classés par catégorie. Seuls les sujets et documents accessibles selon vos droits apparaissent.</p>
    </div>
    ...
    <!-- Loading -->
    <div x-show="loading" class="text-slate-500 text-sm" x-cloak>Chargement de l'arborescence...</div>
            ...
            <div class="border border-slate-200 rounded-lg overflow-hidden bg-white">
                <button type="button" @click="toggleCategory(category.id)"
                    ...
                    <span class="ml-auto text-xs text-slate-500"
                          x-text="countDocuments(category) + ' doc' + (countDocuments(category) > 1 ? 's' : '')"></span>
